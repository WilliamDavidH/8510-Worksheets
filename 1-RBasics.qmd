...
> Combined two statements and strung them together. Also removed quotation marks.
...
>equality 5 is not equal to 10
4 is greater than 3
again true that 5 is not the same as 10
5 is that same as 5 so would be true
...
>multiplied consectutively, so 2 * 2, Sum * 4, etc.
...
>That simply lists the 3rd numeric in the printed results as 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
our.numbers <- c(1, 3, 5, 7)
our.numbers * 1
```
...
>For x there is a list of sequential numerics
For y each numeral is a one
For z is a compination of multiplication and addition so for the last line 5 * 2 + 1 = 11
...
>The scope from the documentation listes it as one data frame with 14 variable and some 60698 observations.
gayguides {DigitalMethodsData}	R Documentation
Bob Damron's Address Books
Description



[Package DigitalMethodsData version 0.0.0.1 Index
...
```
Maybe not but got something else with first ten list.
...
>I asked the wrong question and it gave me the last letters of the alphabet.X I corrected from letters to gayguides!
I ran tail for the last six lines 
...
>Collection, title name, and 500th entry.
...
gayguides$Year[1]
```
Seens like this should be earlier???

(@) How about the latest year? 
```{r}
gayguides$Year[500]
```
Was the last year 1983?
Maybe I am just getting line?
...
>List names, birth date, state, gender, race
Court type, name, title, president, party, 
All official type information
However, unless I missed it, does not list religion or marital status, more personal matters, but influential in casting votes, perhaps from a legal historian perspective.

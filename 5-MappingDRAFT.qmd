...
Boston.Address <- BostonWomenVoters %>% unite(Address, Street.Number.on.April.1, 
    Street.of.Residence.on.April.1, Town.of.Birth, State.or.Province.of.Birth)

Boston.data.coordinates <- Boston.Address %>% geocode(city = Address, state = Address, 
    method = "osm", lat = latitude, long = longitude)
    head(Boston.data.coordinates)
```
#Passing 9,544 addresses to the Nominatim single address geocoder
#Warning: Internal Server Error (HTTP 500).
```{r}
data("BostonWomenVoters")
streetaddresses.BWV <- BostonWomenVoters %>% mutate(BostonWomenVoters, streetaddress = paste0(Street.Number.on.April.1, " ", Street.of.Residence.on.April.1, ",", " ", Town.of.Birth, ",", " ", State.or.Province.of.Birth))

BWV.geocoded <- streetaddresses.BWV %>% geocode(address = streetaddress, method='osm', lat = latitude, long = longitude)
head(BWV.geocoded)
```

#Just my copy for example.
```{r}
rec.data.coordinates <- rec.data %>% geocode(city = city, state = state, method='osm', lat = latitude, long = longitude)
head(rec.data.coordinates)
```
...
```{r}
BostonWomenVoters.coordinates <- BostonWomenVoters %>% geocode(state = State.or.Province.of.Birth)
method ='osm', lat = Street.of.Residence.on.April.1, lon = State.or.Province.of.Birth)
head(BostonWomenVoters.coordinates)
```
#Error: "Street.of.Residence.on.April.1." is not a column name in the input dataset.
...
ggplot() + 
  geom_map( data = usa, map = usa, aes(long, lat, map_id=region)) +
  geom_point(data = gayguides, mapping = aes(x=lon, y=lat, fill="white", color="gray", size = 3)) +  ggtitle("Gayguides - Map of the United States")
```
#Title, changed fill, color, and size so somewhat sucessful!

#SAMPLE from Google Search, I think?
```{r}
library(ggplot2)
...
ggplot() + 
  ...
  geom_point(data = gayguides, mapping = aes(x=lon, y=lat, fill="white", color="gray", size = 2)) +  ggtitle("Gayguides - Map of the United States")
```
...
ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region)) +
  geom_point(data = BostonWomenVoters.coordinates, mapping = aes(x=lon, y=lat))
```
